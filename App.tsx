...
const App: React.FC = () => {
  ...
  const [googleSheetUrl, setGoogleSheetUrl] = useState('https://script.google.com/macros/s/AKfycbxriiHwM4pbfTSpJH9st8dlkO2BFiGvalTd3QNMcLGInnmsPelUMpC_YsKzUNcjyQgd/exec');
  ...
  const handleUploadToSheet = async () => {
    ...
    setConfirmDialog({
      ...
      if (hasError) {
         ...
         payload.emailSubject = `[Cảnh báo] Thông số sai lệch - ${currentMachine.name} - ${inputProductName || "Không rõ"}`;
         ...
         let emailHtml = `
            <div style="font-family: Segoe UI, Arial, sans-serif; color: #333;">
              ...
              <p style="margin: 0 0 4px 0;"><strong>Người kiểm tra:</strong> ${currentUser?.username || 'Không rõ'}</p>              
              ...
              <p style="margin: 0 0 10px 0; color: #d9534f; font-weight: bold;">${outOfStandardCount} thông số out chuẩn / ${totalCheckedCount} thông số kiểm</p>
